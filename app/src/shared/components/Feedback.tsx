 ...
 *
 * ⚠️ 출처를 여기서 나열하지 않는다. 한때 "출처는 DART"라고만 적혀 있었는데
 *    시세(공공데이터포털)와 관보(행정안전부)가 붙으면서 그 문장이 거짓이 됐다.
 *    출처 목록의 정본은 meta.json 의 sources 이고, 설정 > 데이터 출처가 그걸 그대로 보여준다.
 */
export function Disclaimer({ compact = false }: { compact?: boolean }) {
  return (
    <p className="ty-caption" style={{ margin: 0 }}>
      {compact
        ? '투자자문·투자권유가 아닙니다. 공개된 공시·공공데이터를 정리해 보여줍니다.'
        : '이 서비스는 투자자문·투자권유가 아닙니다. 공개된 공시·공공데이터를 정리해 보여줄 뿐이며, 투자 판단과 그 결과는 이용자 본인에게 있습니다. 원문과 다를 수 있으므로 반드시 원문을 확인하세요. 전체 데이터 출처는 설정 > 데이터 출처에서 볼 수 있습니다.'}
    </p>
  )
}
